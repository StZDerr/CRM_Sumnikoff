...
@section('content')
    <div class="max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold">Категории платежей</h1>
            <a href="{{ route('payment-categories.create') }}"
        ...
        <div class="bg-white shadow rounded">
            ...
            <div id="payment-categories-list" data-reorder-url="{{ route('payment-categories.reorder') }}" class="divide-y"
                role="list">
                @forelse($items as $item)
                    <div data-id="{{ $item->id }}" class="flex items-center gap-4 px-4 py-3 hover:bg-gray-50"
                        ...
                        <div class="flex items-center gap-2">
                            <a href="{{ route('payment-categories.edit', $item) }}"
                                class="text-indigo-600 hover:text-indigo-800 text-sm p-2">Редактировать</a>

                            <form action="{{ route('payment-categories.destroy', $item) }}" method="POST"
                ...
                @empty
                    <div class="px-4 py-6 text-gray-500">Пока нет категорий платежей.</div>
                @endforelse
            </div>
        </div>
    </div>
