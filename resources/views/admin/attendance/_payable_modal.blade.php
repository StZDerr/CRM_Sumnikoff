...
<div id="advanceExpenseModal-{{ $report->id }}" class="fixed inset-0 z-50 hidden overflow-y-auto">
    {{-- Backdrop --}}
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="relative w-full max-w-lg transform rounded-lg bg-white shadow-xl transition-all">
            ...
            <div class="flex items-center justify-between border-b px-6 py-4">
                <h3 class="text-lg font-semibold text-gray-900">Выплата аванса: {{ $report->user->name }}</h3>
                <button type="button" class="close-advance-modal text-gray-400 hover:text-gray-600"
                    data-modal-id="advanceExpenseModal-{{ $report->id }}">
            ...
            {{-- Form --}}
            <form class="advance-expense-form" data-report-id="{{ $report->id }}">
                @csrf
                ...
                <div class="space-y-4 px-6 py-4">
                    ...
                    {{-- Сумма --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Сумма аванса</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="2"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            placeholder="Аванс за {{ \Carbon\Carbon::parse($report->month)->locale('ru')->translatedFormat('F Y') }}"></textarea>
                    </div>
                ...
                <div class="flex items-center justify-end space-x-2 border-t bg-gray-50 px-6 py-4">
                    <button type="button"
                        class="close-advance-modal rounded bg-gray-200 px-4 py-2 text-sm font-medium hover:bg-gray-300"
                        data-modal-id="advanceExpenseModal-{{ $report->id }}">
                        Отмена
                    </button>
                    <button type="submit"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const modal = document.getElementById('advanceExpenseModal-{{ $report->id }}');
        ...
        const closeButtons = modal.querySelectorAll('.close-advance-modal');
        ...
        // Отправка формы
        const form = modal.querySelector('.advance-expense-form');
        if (form) {
            form.addEventListener('submit', async function(e) {
                ...
                try {
                    const response = await fetch('{{ route('expenses.store-advance') }}', {
                        method: 'POST',
                        headers: {
