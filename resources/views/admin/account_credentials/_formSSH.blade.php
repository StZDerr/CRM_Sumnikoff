    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">
                {{ isset($accountCredential) ? 'Редактировать доступ' : 'Создать доступ к SSH проекта ' . $project->title }}
            </h1>
        ...
        <div class="bg-white shadow-md rounded-lg border border-gray-200 p-6">
            <form
                action="{{ isset($accountCredential)
                    ...
                    : route('account-credentials.storeSSH', $project) }}"
                method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name"
                            value="{{ old('name', $accountCredential->name ?? 'Доступ к SSH') }}"
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Логин SSH</label>
                        <div class="mt-1 flex space-x-2">
                            {{-- Поле user --}}
                            <input type="text" name="login"
                                value="{{ old('login', isset($accountCredential) ? explode('@', $accountCredential->login)[0] : '') }}"
                            ...
                            <input type="text" name="server_ip"
                                value="{{ old('server_ip', isset($accountCredential) ? explode('@', $accountCredential->login)[1] ?? '46.254.17.235' : '46.254.17.235') }}"
                                class="block w-1/2 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                placeholder="IP сервера" required>
                        </div>
                        <small class="text-gray-400">Введите имя пользователя и IP сервера. В базе они объединятся как
                            user@IP.</small>
                    </div>
