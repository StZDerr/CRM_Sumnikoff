...
@section('content')
    <div class="max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold">Статусы кампаний</h1>
            <a href="{{ route('campaign-statuses.create') }}"
        ...
        <div class="bg-white shadow rounded">
            ...
            <div id="campaign-statuses-list" data-reorder-url="{{ route('campaign-statuses.reorder') }}" class="divide-y"
                role="list">
                @forelse($statuses as $status)
                    <div data-id="{{ $status->id }}" class="flex items-center gap-4 px-4 py-3 hover:bg-gray-50"
                        role="listitem">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between">
                                <div>
                                    <div class="font-medium text-gray-900">{{ $status->name }}</div>
                                    <div class="text-xs text-gray-500">{{ $status->slug ?? '-' }}</div>
                                </div>
                                <div class="text-sm text-gray-400">#{{ $status->sort_order }}</div>
                            </div>
                        </div>

                        <div class="flex items-center gap-2">
                            <a href="{{ route('campaign-statuses.edit', $status) }}"
                                class="text-indigo-600 hover:text-indigo-800 text-sm p-2">Редактировать</a>

                            <form action="{{ route('campaign-statuses.destroy', $status) }}" method="POST"
                                onsubmit="return confirm('Удалить статус кампании?');">
                                @csrf
                                @method('DELETE')
